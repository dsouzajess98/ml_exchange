...
contract ml_exchange {
        ...
        uint public submission_stage_block_size = 2; // 6 weeks timeframe

        uint public reveal_test_data_groups_block_size = 1; // 3 days timeframe

        uint public evaluation_stage_block_size = 4; // 7 days timeframe
        ...
        uint256 public train_dg_revealed = 0;
    ...
    function init1(bytes32[max_num_data_groups/partition_size] _hashed_data_groups, int accuracy_criteria, address organizer_refund_address) public payable {
  ...
  function get_submission_id(
    ...
    // Verify data group hashes
    //assert(sha_data_group(_test_data_groups, _test_data_group_nonces) == hashed_data_groups[testing_partition[test_dg_revealed]]);
    test_dg_revealed += 1;
