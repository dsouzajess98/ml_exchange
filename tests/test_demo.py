...
def test_demo(web3, chain):
    ...
    dbg.dprint("\nDeploy gas: " + str(deploy_receipt["gasUsed"]))
    ...
    for i in range(len(testing_partition)):
        ...
        dbg.dprint("(" + str(testing_partition[i]) + ") Test data,nonce: " + str(test_data[start:end]) + "," + str(
            scd.test_nonce[i]))
    ...
    eval_tx = danku.transact().evaluate_model(0)
    ...
    eval_tx = danku.transact().evaluate_model(1)
    eval_receipt = wait_for_transaction_receipt(web3, eval_tx, timeout=timeout)
    total_gas_used += eval_receipt["gasUsed"]
    dbg.dprint("Eval gas: " + str(eval_receipt["gasUsed"]))
